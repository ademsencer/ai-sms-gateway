...
const apkDownloadName = computed(() =>
  appVersion.value ? `sms-gateway-v${appVersion.value}.apk` : 'sms-gateway-latest.apk'
);
  ...
  <aside class="w-64 bg-gray-900 text-white flex flex-col shrink-0">
    <div class="p-6">
      ...
      <div class="flex items-center gap-2 mt-1">
        ...
        <span v-if="appVersion" class="text-[10px] px-1.5 py-0.5 bg-primary-600/30 text-primary-300 rounded font-mono">v{{ appVersion }}</span>
      </div>
    </div>
    ...
    <div class="border-t border-gray-800 px-4 py-3">
      ...
      >
        ...
        <span>
          Download APK
          <span v-if="appVersion" class="text-xs text-green-500/70 ml-1">v{{ appVersion }}</span>
        </span>
      </a>
      <RouterLink
